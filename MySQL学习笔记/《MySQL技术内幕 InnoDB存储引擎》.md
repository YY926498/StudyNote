...
对数据库中的页进行修改时，一般是现在缓冲区中修改对应数据的页，然后将缓冲区的数据刷新到磁盘中。如果在从缓冲区将页的新版本刷新到磁盘时发生了宕机，那么数据就不能恢复。为了避免发生数据丢失的问题，当前事务数据库系统普遍采用了Write Ahead Log策略，即当事务提交时，先写重做日志，再修改页。当由于宕机而导致数据丢失时，通过重做日志来完成数据的恢复。这就是事务ACID中D的要求。

由于缓冲区不能缓存数据库中的所有数据，同时重做日志也不能无限增大，因此引入Checkpoint技术。

Checkpoint技术的目的是解决如下问题：

- 缩短数据库的恢复时间
- 缓冲池不够用，将脏页刷新到磁盘
- 重做日志不可用时，刷新脏页

当缓冲区不够用时，根据LRU算法会溢出最近最少使用的页，若此页是脏页，那么需要强制执行Checkpoint，将脏页刷新刷会磁盘。

重做日志不可用的情况是因为当前事务数据库系统对重做日志的设计都是循环使用的，并不是让其无限增大的。重做日志被重用的部分是指这些重做日志已经不再需要，即当数据库发生宕机时，数据库恢复操作不需要这部分的重做日志，因此这部分就可以被覆盖重用。若此时重做日志还需要使用，那么必须强制产生Checkpoint，将缓冲区中的页至少刷新到当前重做日志的位置。

### Master Thread工作方式

Master Thread具有最高的线程优先级别。其内部由多个循环组成：主循环、后台循环、刷新循环、暂停循环。Master Thread会根据数据库运行状态在这几个循环中切换。
