...
大多数情况下，避免向远程的数据中心生成数据。如果一定要这么做，就要忍受高延迟，并且需要通过增加重试次数和增大缓冲区来解决潜在的网络分区问题。

如果有跨集群复制的需求，同时禁用了从broker到broker之间的通信以及从生产者到broker之间的通信，必须允许从broker到消费者之间的通信。这是最安全的跨集群通信方式。
...
这种架构用于一个中心Kafka集群对应多个本地Kafka集群的情况。

当消费者需要访问的数据集分散在多个数据中心时，可以使用这种架构。或者每个数据中心只处理自己所在数据中心的数据，也可以使用这种架构。

这种架构的好处在于：数据只会在本地的数据中心生成，而且每个数据中心的数据只会被镜像到中央数据中心一次。只处理单个数据中心的应用程序可以被部署在本地数据中心里，而需要处理多个数据中心的数据的应用程序则需要部署在中央数据中心里。因为数据复制是单向的，而且每个消费者总是从同一个集群读取数据，所以这种架构易于部署、配置和监控。

但有一些局限性，比如一个数据中心的应用程序无法访问另一个数据中心的数据。另外每个区域数据中心的数据都需要被镜像到中央数据中心上。镜像进程会读取每一个区域数据中心的数据，并将它们重新生成到中央集群上。如果出现重名的主题，可以被写到中央集群的单个主题上，也可以写到多个主题上。

#### 多活架构

当有两个或多个数据中心需要共享数据并且每个数据中心都可以生产和读取数据时，可以使用双活架构。

好处在于：可以为就近的用户提供服务，具有性能上的优势，而且不会因为数据的可用性问题在功能方面做出牺牲。第二个好处是冗余和弹性。因为每个数据中心具备完整的功能，一旦一个数据中心发生失效，就可以把用户重定向到另一个数据中心。这种重定向完全是网络的重定向，因此是一种最简单、最透明的失效备援方案。

主要问题在于：如何在进行多个位置的数据异步读取和异步更新时避免冲突。另外数据一致性的问题更为关键：

-   如果用户想A数据中心发送数据，同时从B数据中心读取数据，那么在用户读取B中心数据的时候，A中心的数据还没有镜像到B中心，因此出现数据不一致的情况。此时，一般做法是将用户“粘”在A中心，只允许读取A中心的数据。
-   用户在A数据中心订购一本书a，同时在B数据中心订购一本书b。经过数据镜像后，每个数据中心都包含这两个事件，如何保证哪些事件是正确的？

如果能够很好地处理从多个位置异步读取数据和异步更新数据时发生的冲突问题，这一种架构更好。是一种最具伸缩性、弹性、灵活性和成本优势的解决方案。主要解决避免循环复制、相同用户的请求粘在同一个数据中心以及在发生冲突时解决冲突。

双活镜像的挑战之处在于，每两个数据中心之间都需要进行镜像，而且是双向的。如果有多个，需要维护的镜像进程指数增长。

为了避免循环镜像，相同的事件不能无止境地来回镜像。对于每一个“逻辑主题”，可以在每一个数据中心里为它创建一个单独的主体，并确保不要从远程数据中心复制同名的主题。

#### 主备架构

使用多个集群有时是为了达到灾备的目的。比如在同一个数据中心安装两个集群。平时只使用一个，当提供服务的集群不可用时，切换到第二个集群。两个集群之间的数据相同。

这种集群的不足在于浪费了一个集群。另外不能完全避免丢失数据或无重复数据。
...
延展集群并非多个集群，而是单个集群，因此不需要对延展集群进行镜像。延展集群使用Kafka内置的复制机制在集群的broker之间同步数据。

同步复制是这种架构的最大优势。另一个好处是数据中心及所有的broker都发挥了作用，不存在想主备架构那样的资源浪费。不足之处在于能应对的灾难类型有限，只能应对数据中心的故障，无法应对应用程序或者Kafka故障。同时运维比较复杂。
...
Kafka提供的所有度量指标都可以通过JMX接口来访问。

broker将JMX端口作为整个broker配置信息的一部分保存在ZooKeeper上。在没有端口配置的情况下连接到JMX，可以从ZooKeeper上获取端口信息。`/brokers/ids/<ID>`节点包含了JSON格式的broker信息，里面有JMX对应的主机名和端口。

#### 内部或外部度量

JMX接口提供的内部度量指标，由被监控的应用程序生成。其余的外部度量由客户端或其他第三方应用提供。

#### 应用程序健康检测

确保能够通过简单的健康检测来监控应用程序本身的健康状况，可以通过两种方式来实现：
...
Kafka经常用于收集应用程序和系统的度量指标与日志，然后供中心监控系统使用，可以很好地解耦应用程序和监控系统。但是如果使用这个监控系统来监控Kafka，当Kafka崩溃时，可能无法感知到，因为监控系统的数据流随着消失。一种方法是使用一个单独的监控系统来监控Kafka，这个系统不依赖Kafka提供的数据。另一种方法是如果有多个数据中心，可以将数据中心A的Kafka集群度量指标生成到数据中心B的Kafka集群上，反之亦然。确保Kafka的监控和告警不依赖Kafka本身。

#### 非同步分区

非同步分区的数量指明了作为首领的broker有多少个分区处于非同步状态。可以反映Kafka的很多内部问题，从broker的崩溃到资源的过度消耗。当值大于零食，就应该想办法采取相应的行动。

如果集群里多个broker的非同步分区数量一直保持不变，就说明集群中的某个broker已经离线。整个集群的非同步分区数量等于离线broker的分区数量，而且离线broker不会生成任何度量指标。

在出现这个问题前，先尝试运行默认的副本选举，看能否解决问题。

如果非同步分区的数量是波动的，或者虽然数量稳定但并没有broker离线，说明集群出现了性能问题。

1.  先确认问题是与单个broker有关还是与整个集群有关。
2.  如果多个broker都出现了非同步分区，有可能是集群的问题，也有可能是单个broker的问题。此时可以列出集群的所有非同步分区，并检查它们的共性。如果有一个broker掉线，可以将注意力放在这个broker上。否则集群可能出现问题。

-   集群级别的问题：不均衡的负载和资源过度消耗

    分区或首领的不均衡问题解决起来有点复杂，诊断需要用到broker的以下度量指标
    ...
    在一个均衡的集群里，这些度量指标的数量在整个集群范围内是均等的。

    如果集群的流量出现了不均衡，需要将负载较重的broker分区移动到负载较轻的broker上。

    Kafka集群的另一个性能问题是容量瓶颈。为了诊断容量问题，对如下度量指标进行监控：
    ...
    如果集群的数据复制出现问题，那么集群的用户在生产消息或读取消息时也会出现问题。有必要为这些度量指标定义一个基线，并设定响应的阈值。

-   主机级别的问题：硬件问题，进程冲突，本地配置的不一致

    引起性能衰退的硬件问题不是很明显，需要使用硬件提供的工具来监控硬件的健康状况。能够导致Kafka性能衰退的一个比较常见的硬件问题是磁盘故障。Kafka使用磁盘来存储消息，生产者的性能与磁盘的写入速度由直接关系。

    一个broker的磁盘可能会影响到整个集群的性能。因为生产者客户端会连接到所有的broker上，如果操作得当，这些broker的分区几乎能够均等地分布在整个集群里。如果一个broker性能出现衰退并拖慢了处理请求的速度，就会导致生产者的回压，从而拖慢发给所有broker的请求。

#### broker度量指标

1.  活跃控制器数量：表示broker是否就是当前的集群控制器，值可以是0或1.如果是1，表示broker就是当前的控制器。任何时候，都应该只有一个broker是控制器，而且这个broker必须一致是集群控制器。如果出现两个控制器，会导致管理任务无法正常执行。需要将这两个broker重启，并且不能通过正常的方式重启。如果没有控制器，集群无法对状态的变更做出恰当的响应，状态的变更包括主题或分区的创建和broker故障。解决底层的问题之后，重启集群里的所有broker，重置控制器线程的状态。
    
2.  请求处理器空闲率：Kafka使用两个线程池来处理客户端的请求：网络处理器线程池和请求处理器线程池。broker负载的增长对请求处理器线程池有很大的影响。
    
    请求处理器平均空闲百分比表示请求处理器空闲时间的百分比。数值越低，说明broker的负载越高。如果空闲百分比低于20%，说明存在潜在的问题，低于10%，说明出现性能问题。除了集群的规模太小之外，还有可能是请求处理器线程的数量较小，或者是线程做了不该做的事。
    
3.  主题流入字节：使用`b/s`表示，用于确定何时该对集群进行扩展或开展其他与规模增长相关的工作。所有速率指标都提供了7个属性。
    
    -   EventType：度量的单位，“字节”
    -   RateUnit：速率的时间段，“秒”
    -   OneMinuteRate：前1分钟的平均值
    -   FiveMinuteRate：前5分钟的平均值
    -   FifteenMinuteRate：前15分钟的平均值
    -   MeanRate：从broker启动到目前为止的平均值
    -   Count：代表了从broker启动以来接收到流量的字节总数
    
4.  主题流出字节：消费者从broker读取消息的速率。流出速率也包括副本流量。

5.  主题流入的消息：该指标同主题流入速率一样，能反映集群的不均衡情况。

6.  分区数量：broker的分区数量一般不会经常发生改变，值分配给broker的分区总数。包括broker的每一个分区副本，不管是首领还是跟随者。

7.  首领数量：表示broker拥有的首领分区数量。该指标应在整个集群的broker上保持均等。

8.  离线分区：与非同步分区数量一样，离线分区数量也是一个关键的度量指标。该度量只能由集群控制器提供，显式了集群里没有首领的分区数量。原因有：
    
    -   包含分区副本的所有broker都关闭了
    -   由于消息数量不匹配，没有同步副本能够拿到首领身份（并且禁用了不完全的首领选举）

9.  请求度量指标：由于Kafka协议中有多种不同的请求，每种请求都有相应的度量指标。跟时间有关的指标都是自broker启动以来开始计算的。

#### 主题和分区的度量指标

1.  主题实例的度量指标：与之前的broker度量指标非常相似。
2.  分区实例的度量指标：`Log end offset`和`Log start offset`这两个度量指标分别表示消息的最大偏移量和最小偏移量。
...
#### 日志
broker可以配置成定期向磁盘写入日志。通过记录INFO级别的日志，可以捕捉到broker运行状态的重要信息。可以考虑使用两种日志：`Kafka.controller`，将它设置为INFO级别。这个日志用于记录集群控制器的信息。在任何时候，集群里都只有一个控制器，因此只有一个broker会使用这个日志。日志里包括主题的创建和修改操作、broker状态的变更以及集群的活动。另一个是`Kafka.server.ClientQuotaManager`，也可以设置为INFO级别。用于记录与生产和消费配额活动相关的信息。最好不要把它们记录在broker的主日志文件里。

### 客户端监控

#### 生产者度量指标

1.  生产者整体度量指标：描述生产者各个方面的信息，从消息的批次到内存缓冲区的使用情况。
2.  Per-broker和Per-topic度量指标：

#### 消费者度量指标

1.  Fetch Manager度量指标：包含了与字节、请求和消息速率有关的指标。
2.  Per-broker和Per-topic度量指标：
3.  Coordinator度量指标：
...
数据流式无边界数据集的抽象表示。无边界意味着无限和持续增长。除了没有边界外，事件流模型还有其他一些属性：

-   事件流是有序的
-   不可变的数据记录
-   事件流是可重播的

流式处理是指实时地处理一个或多个事件流。流式处理时一种编程范式，就像请求与响应范式和批处理范式那样。

-   请求与响应范式：这是延迟最小的一种范式，响应时间出于亚毫秒和毫秒之间，响应时间一般非常稳定。这种处理模式一般是阻塞的，应用程序向处理系统发出请求，然后等待响应。
-   批处理：具有高延迟和高吞吐量的特点。处理系统按照设定的时间启动处理进程。
-   流式处理：介于上述两者之间。大部分业务流程是可持续进行的，只要业务报告保持更新，业务产品线能够持续响应，业务流程就可以进行下去，无须等待特定的响应，也不要求在几毫秒内得到响应。
...
-   事件时间：指所追踪事件的发生时间和记录的创建时间。在处理数据流时，事件时间是很重要的。
-   日志追加时间：指事件保存到broker的时间。如果启用了自动添加时间戳的功能，而且记录没有包含时间戳，broker会在接收这些记录时自动添加时间戳。
-   处理时间：指应用程序在收到事件之后要对其进行处理的时间。这个时间戳非常不可靠，应该避免使用它。

需要注意时区问题。整个数据管道应该使用同一个时区，否则操作的结果就会出现混淆。

#### 状态

事件与事件之间的信息被称为“状态”。这些状态一般保存在应用程序的本地变量里。但需要注意持久化最近的状态。流式处理包含以下几种类型的状态：

-   本地状态或内部状态：这种状态只能被单个应用程序实例访问，一般使用内嵌在应用程序里的数据库进行维护和管理。本地状态的优势在于它的速度，不足之处在于它受到内存大小的限制。所以流式处理的很多设计模式是将数据拆分到多个子流，这样可以使用有限的本地状态来处理它们。
-   外部状态：使用外部的数据存储来维护，一般使用NoSQL系统。使用外部存储的优势在于，没有大小的限制，而且可以被应用程序的多个实例访问，甚至被不同的应用程序访问。不足之处在于，引入额外的系统会造成更大的延迟和复杂性。应尽量避免使用外部存储，或者将信息缓存在本地，减少与外部存储发生交互，以此来降低延迟。
...
处理单个事件是流式处理最基本的模式。这个模式也叫map或filter模式，经常被用于过滤无用的事件或者用于状态事件。

这种模式下，应用程序读取流中的事件，修改它们，然后把事件生成到另一个流上。

#### 使用本地状态

如果流式处理应用程序包含本地状态，需要解决下列问题：
...
问题在于外部查找会带来严重的延迟，另外，外部查找无法接受这种额外的负载-流式处理系统每秒钟可以处理10-50万个事件，而数据库正常情况下每秒钟只能处理1万个事件，所以需要伸缩性更强的解决方案。

如果能够捕捉到数据库的变更事件，并形成事件流，流式处理作业就可以监听事件流，并及时更新缓存。捕捉数据库的变更事件并形成事件流，这个过程被称为CDC-变更数据捕捉。这种方案叫做流和表的连接。

#### 流和流的连接

要连接两个流，就是在连接所有的历史事件-将两个流里具有相同键和发生在相同时间窗口内的事件匹配起来。流和流的连接也叫做基于时间窗口的连接。
...
Streams API通常将聚合结果写到主题上，这些主题一般是压缩日志主题，即只保留每个键的最新值。

#### 重新处理

一个很重要的模式是重新处理事件，该模式有两个变种：

-   对流式处理应用进行了改进，使用新版本应用处理同一个事件流，生成新的结果，并比较两种版本的结果，然后在某个时间点将客户端切换到新的结果流上
-   现有流式处理应用出现缺陷，修复缺陷之后，重新处理事件流并重新计算结果

对于第一种情况，Kafka将事件流长时间地保存在可伸缩的数据存储里。要是用两个版本的流式处理应用来生成结果，只需要满足如下条件：

-   将新版本的应用作为一个新的消费者群组
-   让它从输入主题的第一个偏移量开始读取数据
-   检查结果流，在新版本的处理作业赶上进度时，将客户端应用程序切换到新的结果流上

对第二种情况，需要“重置”应用，让应用回到输入流的起始位置开始处理，同时重置本地状态（不能讲两个版本应用的处理结果混淆起来），而且还可能需要清理之前的输出流。

### Kafka Streams的架构概览

#### 构建拓扑

每个流式应用程序至少会实现和执行一个拓扑。拓扑是一个操作和变换的集合，每个事件从输入到输出都会流经它。拓扑是由处理器组成的，这些处理器是拓扑图里的节点。数据源处理器从主题上读取数据，并传给其他组件，而数据池处理器从上一个处理器接收数据，并将它们生成到主题上。

#### 对拓扑进行伸缩

Streams引擎将拓扑拆分成多个子任务来并行执行。拆分成多少个任务取决于Streams引擎，同时也取决于主题的分区数量。每个任务负责一些分区：任务会订阅这些分区，并从分区读取事件流，在将结果写到数据池之前，在每个事件上执行所有的处理步骤。

流式处理的伸缩方式：主题里有多少分区，就会有多少任务。Streams将连接操作所涉及的分区全部分配给相同的任务，这样，这个任务就可以从相关的分区读取数据，并独立执行连接操作。Streams要求同一个连接操作所涉及的主体必须要有相同数目的分区，而且要基于连接所使用的键进行分区。

如果需要进程重新分区，再执行任务，此时在任务之间形成依赖。Streams通过使用新的键和分区将时间写到新的主题来实现重新分区，并启动新的任务从新主题上读取和处理事件。这样减少管道各个部分之前的依赖。