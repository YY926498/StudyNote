...
分区用多副本机制来提升可靠性，但只有leader副本对外提供读写服务，而follower副本只负责在内部进行消息的同步。如果一个分区的leader副本不可用，就意味着整个分区变得不可用，此时需要Kafka从剩余的follower副本中挑选一个新的leader副本来继续对外提供服务。

在创建主题的时候，该主题的分区及副本会尽可能均匀地分布到Kafka集群的各个broker节点上，对应的leader副本的分配也比较均匀。

当分区的leader节点发生故障时，其中一个follower节点就会成为新的leader节点，这样会导致集群的负载不均衡，从而影响整体的健壮性和稳定性。当原来的leader节点恢复之后重新加入集群时，只能成为一个新的follower节点而不再对外提供服务。

为了能够有效地治理负载失衡的情况，Kafka引入优先副本的概念。所谓优先副本是指在AR集合列表中的第一个副本。比如一个主题中分区0的AR集合列表为[1,2,0]（表示broker编号），那么分区0的优先副本即为1号broker。理想情况下，优先副本就是该分区的leader副本。Kafka要确保所有主题的优先副本在Kafka集群中均匀分布，这样就保证了所有分区的leader均衡分布。如果leader分布过于集中，就会导致集群负载不均衡。

优先副本的选举是指通过一定的方式促使优先副本选举为leader副本，以此来促进集群的负载均衡，这一行为也可以称为“分区平衡”。

分区平衡并不意味着Kafka集群的负载均衡，还要考虑集群中的分区分配时候均衡。另外每个分区的leader副本的负载也是各不相同的。

Kafka中提供分区自动平衡的功能，默认开启。该功能开启时，Kafka控制器会启动一个定时任务，这个定时任务会轮询所有的broker节点，计算每个broker节点的分区不平衡率（broker中的不平衡率=非优先副本的leader个数/分区总数）时候超过设定参数（默认10%），如果超过设定的参数则会自动执行优先副本的选举动作以求分区平衡。执行周期由参数控制，默认300秒。

分区自动平衡的功能可能会引起客户端一定时间的阻塞。因为执行的时间无法自主掌控。在关键时期执行优先副本的自动选举操作，会有业务阻塞、频繁超时之类的风险。因此可以设置相关监控，在合适的时机手动执行分区平衡。

leader副本的转移是一项高成本的工作，如果要执行的分区数很多，必然会对客户端造成一定的影响、如果集群中包含大量的分区，那么在对所有分区都执行一遍优先副本的选举有可能失效。因为具体的元数据信息会存放到ZooKeeper的节点上，如果这些数据超过了ZooKeeper节点所允许的大小，那么选举就会失败。

另外一种是对部分分区执行优先副本的选举操作。这样可以自己控制分区。
